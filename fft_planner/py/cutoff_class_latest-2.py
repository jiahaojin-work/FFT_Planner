...
class FFT_plan:
    ...
    def ifft2_direct(self, freq):
        """
        直接用二维逆 DFT 的定义计算 ifft2：
        x[n,m] = (1/(M*N)) * sum_{k=0..M-1} sum_{l=0..N-1} F[k,l] * exp(2j*pi*(k*n/M + l*m/N))
        仅用于小尺寸验证。
        """
        
        freq = np.asarray(freq)
        M, N = freq.shape[-2], freq.shape[-1]
        print("M N ", M, N)
        # indices
        n = np.arange(M)[:, None]    # (M,1)
        m = np.arange(N)[None, :]    # (1,N)
        k = np.arange(M)[:, None]    # reuse for outer product
        l = np.arange(N)[None, :]

        # 构造因子矩阵（向量化）
        # Wm: shape (M, M) with entries exp(2j*pi * k*n / M)
        # Wn: shape (N, N) with entries exp(2j*pi * l*m / N)
        Wm = np.exp(2j * np.pi * (k * n.T) / M)  # careful with broadcasting shapes
        # 上面写法有点绕，下面更直观且正确：
        k_idx = np.arange(M)[:, None]  # (M,1)
        n_idx = np.arange(M)[None, :]  # (1,M)
        Wm = np.exp(2j * np.pi * (k_idx @ n_idx) / M)  # (M, M)
        print("test wm", type(Wm), Wm.shape)
        print(Wm)
        l_idx = np.arange(N)[:, None]  # (N,1)
        m_idx = np.arange(N)[None, :]  # (1,N)
        Wn = np.exp(2j * np.pi * (l_idx @ m_idx) / N)  # (N, N)
        print("test wn", type(Wn), Wn.shape)
        print(Wn)
        # 通过矩阵乘法实现双和： X = (1/(M*N)) * Wm.T @ F @ Wn
        # 注意维度：Wm is (M,M), freq is (M,N), Wn is (N,N)
        X = Wm.T @ freq 
        Y = X @ Wn.T
        # print("test res")
        # print("wm transpose")
        ...
        wmt_block = np.array(Wm_block.T)  # 显式计算转置
        wnt_block = np.array(Wn_block.T)

        X_block = wmt_block @ freq_block
        Y_block = X_block @ wnt_block

        # print("=== 3x3 Block Calculation ===")
        # print("Wm block (3x3):")
        # print(Wm_block)
        # print("freq block (3x3):")
        # print(freq_block)
        # print("Wn block (3x3):")
        # print(Wn_block)
        # print("Wm transpose block (3x3):")
        # print(wmt_block)
        # print("Wn transpose block (3x3):")
        # print(wnt_block)
        # print("Intermediate result X_block (3x3):")
        # print(X_block)
        print("Final result Y_block (3x3):")
        print(Y_block)
