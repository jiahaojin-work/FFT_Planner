...
class FFT_plan:
    def __init__(self, mode=0):
        ...
        print("safe__type ", type(self.safe[0]))
        self.rotation = None
    ...
    def my_ifft2(self, freq):
        # 第一步：按行做 1D IFFT
        tmp = np.fft.ifft(freq, axis=0)
        # 第二步：按列做 1D IFFT
    ...
    def FFT_process(self, processed, img, j):
        ...
        # print("------------------ freq1 -------------------------")  
        # print(freq1)
        t = np.fft.ifft2(freq1)
        print("------------------ ifft2 -------------------------")  
        print(t)
        print("--------------------ifft sefllf ------------------")
        print(self.my_ifft2(freq1))
        # print('self.shape', self.shape)
        # filtered_image = np.abs(np.fft.ifft2(freq1))[0:self.shape[0], 0:self.shape[1]]
        filtered_image = np.abs(np.fft.ifft2(freq1))
        
        ifft = np.fft.ifft2(freq1)
        rows, cols = freq1.shape
        # print('freq1 shape', freq1)
        print("py ifft * (rows*cols) first 2x3:\n", (ifft * (rows*cols))[:2,:3])
        print("py ifft first 2x3:\n", ifft[:2,:3])
        if self.mode == 2 or self.mode == 3:
            plt.subplot(4, 2, j + 1 + 2)
            print('getestewtwe')
            print(filtered_image / 255)
            ...
            # plt.show()
            if j == 0:
                plt.imsave("/home/jack/" + str(NUM) + "filtered_image.png", filtered_image / 255 * self.dis_max, cmap="gray")
        ...
        # ***************
        if j == 0:
            # opencv 的fft filtered_image是一个二维矩阵
            ...
            freq1 *= self.H_S[j]
            filtered_image = np.abs(np.fft.ifft2(freq1))[0:self.shape[0], 0:self.shape[1]]
            # print("filtered_image type ", type(filtered_image))
            print("---------- j = 0 -----------------")
            print(gray_image)
            self.solver.gradient = filtered_image
            print("solver gradient type ", type(self.solver.gradient))
            ...
            print("greater0 ", type(greater0), greater0)
            print("greater1 ", type(greater1), greater1)
    ...
    def FFT_parallel(self, msg_lidar, msg_odom, target):
        ...
        # print('imgidx type', type(imgidx))
        # print('target type', type(target))
        # print('msg_odom type', type(msg_odom))
        ...
        # print('rotation type', type(self.rotation))

        [roll, pitch, yaw] = self.rotation.as_euler('xyz', degrees=False)
        # print("test euler ", roll, pitch, yaw)
        # 一种粗糙的计算视野重叠率的方式
        F_pitch = max(0, 1 - abs(pitch) / self.FOV[0])
        # print('f pitch test ', self.FOV[0], F_pitch)
        ...
        # print('f total test', A_roll, F_roll, F_total)
        if F_total < 0.5:
    ...
    with rosbag.Bag('/home/jack/2025-05-22-17-59-20.bag', 'r') as lidar_msg:
        for topic, msg, t in lidar_msg.read_messages():
            ...
            if msg_lidar is not None and msg_odom is not None:
                # print("-------msg lidar--------------")
                np.set_printoptions(threshold=np.inf)  # 打印时不省略
                # print(msg_lidar)
                # print('msg  lidar type', type(msg_lidar))
                # print('msg  lidar type', msg_lidar.shape)
                
                # print("-------msg odom--------------")
                # print(msg_odom)
                
                since = time.time()
                # msg lidar msg odom 转成陪同数据结果， target坐标系是全局坐标系0,0,0 -> x,y,z
