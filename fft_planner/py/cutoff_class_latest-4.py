...
class FFT_plan:
    ...
    def cal_forward_kinematics_divide(self, pl, vl, al, p1, times, tao, T1, T2, p1_safe=True):
        ...
        print('test tao1', tao1)
        print('test tao2', tao2)
        # 通过时间二分法迭代
    ...
    def cal_best_idx_end(self, p0, v0, a0, p2, v2):
        ...
        for iter in range(self.iter):  # 此处迭代次数太少太多都会出问题
            ...
            for idx in range(3):  # 三轴
                ...
                print('prepare kalpha1 ', kalpha1)
                print('prepare balpha1 ', balpha1)
                print('prepare kbeta1 ', kbeta1)
                print('prepare bbeta1 ', bbeta1)
                print('prepare kgamma1 ', kgamma1)
                print('prepare bgamma1 ', bgamma1)
                print('prepare a1 ', a1)
                print('prepare b1 ', b1)
                ...
                if np.any(p2 == -1):
                    ...
                    print('prepare kalpha2', kalpha2)
                    print('prepare balpha2', balpha2)
                    print('prepare kbeta2', kbeta2)
                    print('prepare bbeta2', bbeta2)
                    print('prepare kgamma2', kgamma2)
                    print('prepare bgamma2', bgamma2)
                ...
                print(str(idx) + "param_alpha", param_alpha)
                print(str(idx) + "param_beta", param_beta)
                print(str(idx) + "param_gamma", param_gamma)
            ...
            # 更新迭代量
            # 计算三轴的参数
            print('1025 test alpha ', param_alpha)
            print('1025 test beta', param_beta)
            print('1025 test gamma', param_gamma)
            print('1025 test psol', p_sol)
            print('param aplha 0 ', param_alpha[0])
            ...
            s1, times1, pl1, vl1, al1, tao1 = self.cal_sva(alpha1, beta1, gamma1, a0, v0, p0, T1)
            print('test al1 type ', type(al1), type(al1[-1]))
            a_sol = np.array(al1[-1])
            print('a sol', a_sol)
            if not np.any(p2 == -1):
                # 计算第二段路程
                print('param alhpa 1 ', param_alpha[1])
    ...
    def find_nearest_margin(self, matrix, ref_index, terminal_index, r_safe, r_extreme):
        ...
        print(matrix)
        print('ref index', type(ref_index))
        print(ref_index)
        
        print('terminal index', type(terminal_index))
        print(terminal_index)
    
        print('rsafe', type(r_safe))
        print(r_safe)
        
        print('r extreme', type(r_extreme))
        print(r_extreme)
        
        
        '''起始点至终止点的广度优先搜索，寻找距离目标点最近的安全域边界，步长为安全域半径'''
        ...
        print('bfs ', r_extreme, r_safe, kx, ky)
        print('bfs ', x_min, x_max, y_min, y_max)
    ...
    def find_nearest_one_origin(self, matrix, ref_index):
        ...
        print("ref index test", ref_index)
    ...
    def FFT_parallel(self, msg_lidar, msg_odom, target):
        ...
        if (self.target_distance < self.cutoff[0] and self.safe[0][ref[0], ref[1]] == 1):
            ...
            s, times, p, v, a, tao = self.cal_sva(alpha, beta, gamma,
                                                  self.linar_a, self.linar_v, np.zeros(3), T)
            print(type(s), type(times), type(v), type(a), type(a), type(tao))
            if np.max(tao) > 1 + 1e-2:
        ...
        if np.max(self.safe[0]) < 0.01:
            ...
            if not self.approaching:
                ...
                if ref_endpt is not None and self.safe[1][ref_endpt[0], ref_endpt[1]] and self.safe[0][ref_endpt[0], ref_endpt[1]]:
                    ...
                    if len(processed[1]) > 0:
                        ...
                        print('all possible lines ', all_possible_lines)
                ...
                if not np.any(ref_index[1] == -1):  # 远端规划成功
                    ...
                    print('angle_horizontal', angle_horizontal)
            ...
            print("CalBestIdxEnd res p sol ", p_ref)
            print("CalBestIdxEnd res pl ", pl)
            print("CalBestIdxEnd res vl ", vl)
            print("CalBestIdxEnd res al ", al)
            print("CalBestIdxEnd res times ", times)
            print("CalBestIdxEnd res T1 ", T1)
            print("CalBestIdxEnd res T2 ", T2)
            print("CalBestIdxEnd res v1 ", v1)
            print("CalBestIdxEnd res tao ", tao)
            print("CalBestIdxEnd res s all ", s_all)
            ...
            print('angle_horizontal1', angle_horizontal1, self.mx_centre[1] - angle_horizontal1 / self.angle_ptx)
            ...
            print("e_ref_local ", e_ref_local, self.mx_centre[0] + (angle_vertical1 - np.pi / 2) / self.angle_ptx)
            print('ref_index_theory',  ref_index_theory[0])
...
if __name__ == '__main__':
    ...
    with rosbag.Bag('/home/jack/2025-05-22-17-59-20.bag', 'r') as lidar_msg:
        for topic, msg, t in lidar_msg.read_messages():
            if jmp_cnt > 0:
